...
function useTheForceNodeLinkMan(
  links: Array<d3.SimulationLinkDatum<d3.SimulationNodeDatum & HappyNode> & HappyLink>,
  d3gg: typeof gg,
) {
  return d3gg
    .forceLink<HappyNode, d3.SimulationLinkDatum<d3.SimulationNodeDatum & HappyNode> & HappyLink>(
      links,
    )
    .id((node) => node.id)
    .distance(1);
...
function chart(data: MiserableNodesLinks): SVGSVGElement {
  ...
  const simulation = d3
    ...
    .force('link', useTheForceNodeLinkMan(links, d3))
  ...
  */
  ...
  const node = svgContainerGroupG
    ...
    .on('click', (event, d) =>
      clicked(
        ...
        // - FIXME:
        getSuperZoomScrollBehaviour(svgContainerGroupG) as any,
      ),
    );
  ...
  // - FIXME: 
  svg.call(getSuperZoomScrollBehaviour(svgContainerGroupG) as any) ;
  ...
  simulation.on('tick', () => {
    ...
    ((node as d3.Selection<any, HappyNode & SimulationNodeDatum, any, any>).attr(
      'cx',
      (d: HappyNode & SimulationNodeDatum) => d?.x ?? 0,
    ...
    svg
      ...
      .call(
        ...
        d3.zoomIdentity.translate(
          ...
          translate[0] - parseFloat(d3.select(escapedID).attr('cx')) ?? 0,
          translate[1] - parseFloat(d3.select(escapedID).attr('cy')) ?? 0,
        ),
      ); // updated for d3 v4
  }
